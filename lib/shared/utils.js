...
function extractStyling(text) {
  const regexReplace = [
    [/^<br>/, ''], // remove <br> from beginning
    [/<br>/g, '\n'], // replace all other <br> with new line
    [/<.*?>/g, ''], // remove all <*> tags
    [/{.*?}/g, ' '], // replace all '{*}' with a white space
    [/(>|<|{|})/g, ''], // remove all remaining '<', '>', '{', '}' characters
    [/ {2,}/g, ' '], // replace all 2+ length white space with a single whitespace
    [/^\s+|\s+$/gm, ''], // trim every line
  ];
